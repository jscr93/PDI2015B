...
CCSMetaCanvas::CCSMetaCanvas(CDXGIManager *pOwner)
{
	m_pCB = 0;
	m_pCS = 0;
	m_pInput_1 = 0;
...
bool CCSMetaCanvas::Initialize()
{
	m_pCS = m_pOwner->CompileCS(L"..\\Shaders\\MetaCanvas.hlsl", "Main");
	if (!m_pCS)
		return false;
...
void CCSMetaCanvas::Configure()
{
	...
	ID3D11UnorderedAccessView *pUAV[2] = { 0, 0 };
	...
	m_pOwner->GetDevice()->CreateUnorderedAccessView(m_pOutput_1, NULL, &pUAV[0]);
	m_pOwner->GetDevice()->CreateUnorderedAccessView(m_pOutput_2, NULL, &pUAV[1]);
	m_pOwner->GetContext()->CSSetShaderResources(0, 2, pSRV);
	m_pOwner->GetContext()->CSSetUnorderedAccessViews(0, 2, pUAV, NULL);
...
void CCSMetaCanvas::Execute()
{
	D3D11_TEXTURE2D_DESC dtd;
	m_pOutput_1->GetDesc(&dtd);
	int gx, gy;
	gx = (dtd.Width + 7) / 8;
	gy = (dtd.Height + 7) / 8;
